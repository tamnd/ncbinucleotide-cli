...
describes what users get. Pick a format with `-o`, or let ncbinucleotide choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
ncbinucleotide <command> -o table   # aligned columns for reading
ncbinucleotide <command> -o jsonl   # one JSON object per line, for piping
ncbinucleotide <command> -o json    # a single JSON array
ncbinucleotide <command> -o csv     # spreadsheet friendly
ncbinucleotide <command> -o tsv     # tab-separated
ncbinucleotide <command> -o url     # just the URL column
ncbinucleotide <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
ncbinucleotide <command> --fields id,title,url
```
...
```bash
ncbinucleotide <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
ncbinucleotide <command>            # a table, because this is a terminal
ncbinucleotide <command> | wc -l    # JSONL, because this is a pipe
```
